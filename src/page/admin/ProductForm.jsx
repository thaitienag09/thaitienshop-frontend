...
// Replaced Firebase Storage with Cloudinary for free tier usage
import {
    ChevronLeft,
    Save,
    Image as ImageIcon,
...
export default function ProductForm() {
    ...
    const handlePreviewUpload = async (e, index) => {
        const file = e.target.files[0];
        if (!file) return;
    ...
    const removePreviewField = (index) => {
        setFormData(prev => ({
            ...prev,
    ...
    const handlePreviewTitleChange = (index, value) => {
        setFormData(prev => {
            const newPreviews = [...prev.previews];
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => {
            ...
            // Auto-generate slug from title if not in edit mode and ID hasn't been manually touched much?
            // For simplicity, let's just update ID if title changes and ID is empty or matches previous auto-gen
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Data prepared for Firestore:', dataToSave.title);

            // Clean up internal ID from dataToSave before Firestore write if it's there
            const { id: _, ...cleanedData } = dataToSave;
            ...
            if (isEditMode) {
                ...
                console.log('Creating new project with ID:', customId);
                await setDoc(doc(db, 'projects', customId), finalData);
            }

            console.log('Firestore write successful! Navigating...');
            navigate('/admin/products');
        } catch (error) {
